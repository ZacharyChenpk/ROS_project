    ...
    def _build_net(self):
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer, trainable):
            with tf.variable_scope('l1'):
                w1 = tf.get_variable('w1', [self.n_features, 64], initializer=w_initializer, collections=c_names, trainable=trainable)
                b1 = tf.get_variable('b1', [1,64], initializer=b_initializer, collections=c_names,  trainable=trainable)
                l1 = tf.nn.relu(tf.matmul(s, w1) + b1)

            with tf.variable_scope('l2'):
                w2 = tf.get_variable('w2', [64, 64], initializer=w_initializer, collections=c_names,  trainable=trainable)
                b2 = tf.get_variable('b2', [1, 64], initializer=b_initializer, collections=c_names,  trainable=trainable)
                l2 = tf.matmul(l1, w2) + b2

            with tf.variable_scope('l3'):
                w3 = tf.get_variable('w3', [64, 256], initializer=w_initializer, collections=c_names,  trainable=trainable)
                b3 = tf.get_variable('b3', [1, 256], initializer=b_initializer, collections=c_names,  trainable=trainable)
                l3 = tf.matmul(l2, w3) + b3

            with tf.variable_scope('l4'):
                w4 = tf.get_variable('w4', [256, self.n_actions], initializer=w_initializer, collections=c_names,  trainable=trainable)
