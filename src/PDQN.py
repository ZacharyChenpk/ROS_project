...
class Memory(object):  # stored as ( s, a, r, s_ ) in SumTree
    ...
    def sample(self, n):
        ...
        if self.tree.tree[-1]!=0:
            ...
            min_prob = np.min(self.tree.tree[-self.tree.capacity:self.tree.data_pointer + self.tree.capacity-1]) / self.tree.total_p     # for later calculate ISweight
        for i in range(n):
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            e1 = tf.layers.dense(e1, 64, tf.nn.relu, kernel_initializer=w_initializer,
                                 bias_initializer=b_initializer, name='e1_2')
            e1 = tf.layers.dense(e1, 256, tf.nn.relu, kernel_initializer=w_initializer,
                                 bias_initializer=b_initializer, name='e1_3')
            self.q_eval = tf.layers.dense(e1, self.n_actions, kernel_initializer=w_initializer,
                                          bias_initializer=b_initializer, name='q')
            a_predict = tf.argmax(self.q_eval, 1)
            tf.add_to_collection('a_out', a_predict)
        ...
        with tf.variable_scope('target_net'):
            ...
            t1 = tf.layers.dense(t1, 64, tf.nn.relu, kernel_initializer=w_initializer,
                                 bias_initializer=b_initializer, name='t1_2')
            t1 = tf.layers.dense(t1, 256, tf.nn.relu, kernel_initializer=w_initializer,
                                 bias_initializer=b_initializer, name='t1_3')
